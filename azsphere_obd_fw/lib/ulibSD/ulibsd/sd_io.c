...
BYTE __SD_Send_Cmd(BYTE cmd, DWORD arg)
{
	...
	BYTE data[6] = { cmd, (BYTE)(arg >> 24), (BYTE)(arg >> 16), (BYTE)(arg >> 8), (BYTE)(arg >> 0), crc };
...
SDRESULTS __SD_Write_Block(SD_DEV* dev, void* dat, BYTE token)
{
	...
	{
		// Send block data
		for (idx = 0; idx != SD_BLK_SIZE; idx++) SPI_Write(*((BYTE*)dat + idx));
		/* Dummy CRC */
		SPI_Write(0xFF);
		SPI_Write(0xFF);
		// If not accepted, returns the reject error
		if ((SPI_Read() & 0x1F) != 0x05) return(SD_REJECT);
	}
...
DWORD __SD_Sectors(SD_DEV* dev)
{
	...
	if (__SD_Send_Cmd(CMD9, 0) == 0)
	{
		// Wait for response
		while (SPI_Read() == 0xFF);
		for (idx = 0; idx != 16; idx++) csd[idx] = SPI_Read();
		// Dummy CRC
		SPI_Write(0xFF);
		SPI_Write(0xFF);
		SPI_Release();
