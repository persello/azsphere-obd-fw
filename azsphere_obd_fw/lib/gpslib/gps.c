...
int GPSThreadTerminationRequest = 0;

void* GPSThreadMain(void* _param) {

	while (!GPSThreadTerminationRequest) {
		...
		if (!GPSThreadLock) {
			...
			// Same as while(1) but exits on thread exit or lock request
			while (!GPSThreadTerminationRequest && !GPSThreadLock) {
			...
			// Log_Debug("GPS: NMEA sentence is \"%s\".\n", sentence);
